...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}[node distance=60mm, on grid]
        ...
        \node[draw, rectangle, fill=lightblue!50, minimum width=80mm, minimum height=15mm] (Q) 
            {$Q_n^j = \left(\int y_n^j (w^j)^{\frac{\sigma_j-1}{\sigma_j}} d w^j \right)^{\frac{\sigma_j}{\sigma_j - 1}} = C_n^j + \sum_{k=1}^J \int M_n^{j,k}(w^k)dw^k$};
        ...
        \draw[->, very thick, >=Latex, bend left=30] (Q) to 
            node[midway, below left=1mm, align=center, text width=70mm] 
        ...
        \draw[->, very thick, >=Latex, bend left=40] (Q) to 
            node[midway, above right=1mm, align=center, text width=70mm] 
